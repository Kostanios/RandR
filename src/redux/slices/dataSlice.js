...
export const logInThunk = createAsyncThunk(LOG_IN, async ({ getState }) => {
  ...
  const response = await DataAPI.logIn(phoneNumber);
...
export const confirmJwtThunk = createAsyncThunk(CONFIRM_JWT, async (jwt, { getState }) => {
  ...
  const response = await DataAPI.confirmJwt(jwt);
  return response.data;
})
...
export const confirmOtpThunk = createAsyncThunk(CONFIRM_OTP, async (otp, { getState }) => {
  ...
  const response = await DataAPI.confirmOtp({ phoneNumber, otp });
  return response.data;
})
...
export const authSlice = createSlice({
  ...
  initialState: {
    ...
    isOtpSent: false,
    isLogined: false,
    currentComponent: COMPONENTS_LOG_IN
  },
  reducers: {
    [setJwt]: (state, action) => {
